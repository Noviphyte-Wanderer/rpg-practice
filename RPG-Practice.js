...
const locationSet = [
  ...
  },
  {
    ...
    colorPalette: () => {
      document
        .querySelector(":root")
        .style.setProperty("--page-background", "#253428");
      document
        .querySelector(":root")
        .style.setProperty("--textbox-color", "#545");
  ...
  },
  {
    ...
    colorPalette: () => {
      document
        .querySelector(":root")
        .style.setProperty("--page-background", "#a09050");
      document
        .querySelector(":root")
        .style.setProperty("--textbox-color", "#a09000");
      document.querySelector(":root").style.setProperty("--game-text", "#fff");
    },
  },
];
...
function renderButtons(location) {
  ...
  for (let i = 0; i < location.buttons.length; i++) {
    sectionSelectors.controlSection.innerHTML +=
      location.buttons[i].renderBtn();
  }
  for (let i = 0; i < location.buttons.length; i++) {
    document
      .getElementById(location.buttons[i].getID())
      .addEventListener("click", location.buttons[i].getFunction());
  }
}
...
function buyWeapons() {
  ...
  sectionSelectors.storeSection.insertAdjacentHTML(
    "beforeend",
    `
  ...
  for (let i = 0; i < 8; i++) {
    weaponsSection.insertAdjacentHTML(
      "beforeend",
      `
...
function renderList(sectionID, parentSection) {
  parentSection.insertAdjacentHTML(
    "beforeend",
    `
    <div id="${sectionID}">
    </div>
